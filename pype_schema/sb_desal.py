...
def get_tags(taglist_path, save_path=None):
    ...
    for i, row in taglist.iterrows():
        ...
        if row["id"] == row["id"]:
            inv_tag_map[row["Tag"]] = (
                f'[{int(row["id"])}] {row["iGreen database tag name"]}'
            )
        else:
...
def plot_flow_loss(
    flows_series, time_series, net, tagMap, inv_tag_map, days=1, start_date=0, shift=0
):
    loss_tag = net.get_tag("FlowLostWithinIntermediateTanksAndFilters", recurse=True)
    lossRatio_tag = net.get_tag(
        "FlowLostWithinIntermediateTanksAndFiltersRatio", recurse=True
    )
    ...
    fig, axs = plt.subplots(
        2, 1, sharex=True, gridspec_kw={"height_ratios": [1, 1]}, figsize=(6, 6)
    )
    ax1 = axs[0]
...
def plot_MPD_leakage_and_BW(
    flows_series, time_series, net, tagMap, inv_tag_map, days=0, start_date=0, shift=0
):
...
def estimate_volumn(
    ...
    fig, axs = plt.subplots(
        2, 1, sharex=True, gridspec_kw={"height_ratios": [1, 1]}, figsize=(8, 6)
    )
    ax1 = axs[0]
